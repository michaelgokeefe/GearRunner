...
public static class VZPlugin
{
   ...
   [DllImport("vzplugin")]
   static extern void VZReprogramBike(IntPtr program);

   public static void VZReprogramBike(string program)
   {
      IntPtr _program = Marshal.StringToHGlobalAnsi(program);
      VZReprogramBike(_program);
      Marshal.FreeHGlobal(_program);
   }

   [DllImport("vzplugin")]
   public static extern float VZReprogramComplete();
